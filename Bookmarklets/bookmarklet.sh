...
if type ruby &>/dev/null; then
	echo -n "javascript:$(cat "$1" | "$bin" -nc | ruby -pe '$_.gsub!(/([^a-zA-Z0-9_\.\-]+)/n) { "%" + $1.unpack("H2" * $1.size).join("%").upcase }')"
elif type php &>/dev/null; then
	echo -n "javascript:$(cat "$1" | "$bin" -nc | php -r 'echo rawurlencode(file_get_contents("php://stdin"));')"
else
