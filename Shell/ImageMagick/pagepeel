#!/bin/bash
#
# Developed by Fred Weinhaus 7/28/2011 .......... 7/31/2011
#
...
#                           curve; 0<=integer<=100; default=30
...
curv=30			# 100 times ratio of depth/width of parabola
...
if [ "$mode" = "parabola" ]; then
	...
	# but a=1/(4p)
	# so a=.01*curv/(xc-xr)
	a1=`convert xc: -format "%[fx:0.01*$curv/($xc-$xr)]" info:`
	# compute b and c 
	# (yc - a*xc^2) = b*xc + c
	# (yr - a*xr^2) = b*xr + c
	# c = (yc - a*xc^2) - b*xc 
	...
	# p=100*(yc-yb)/(4curv)
	# but a=1/(4p)
	# so a=0.01*curv/(yc-yb)
	a2=`convert xc: -format "%[fx:0.01*$curv/($yc-$yb)]" info:`
	# compute b and c 
	# (xc - a*yc^2) = b*yc + c
	# (xb - a*yb^2) = b*yb + c
	# c = (xc - a*yc^2) - b*yc 
	...
	# phi=angle between radius at xc and arc line = atan(w/h) = atan(1/(0.1*curv))
	# theta=pi-2*phi
	# R=half-width/sin(theta)

	# compute radius of arc between xc and xr and curv
	...
	# compute radius of arc between yc and yb and curv
...
if [ "$shadow" != "0" ]; then
	...
	convert $tmpA1 \
		...
		\( -clone 3 -channel All -blur 0x$shadow $writeA3 \) \
		...
		\( -clone 6 -clone 3 -compose copy_opacity -composite $writeA4 \) \
