#!/bin/bash
#
# Developed by Fred Weinhaus 10/12/2011 .......... revised 10/12/2011
# 
# USAGE: levels [-C colorspace ] [-i invals] [-o outvals] [-r range] [-g gammaval] [-c clipvals] infile outfile
	...
	while [ $# -gt 0 ]
	  ...
	  -h|-help)    # help information
		 	...
		 	-r)    # range
				   ...
				   case "$range" in 
						percent|p) channels="percent" ;;
						8bit|8) channels="8bit" ;;
						qrange|q) channels="qrange" ;;
...
#echo "clipping=$clipping; channeling=$channeling"

if [ "$channels" = "lum" ]; then
	convert $tmpA1 -colorspace YUV -separate \
		\( -clone 0 $clipping \
		-level ${invals},${gammaval}${unit} -clamp \
		+level ${outvals}${unit} \) \
		-delete 0 -insert 0 \
		-set colorspace YUV -combine -colorspace RGB \
		$outfile

else
	convert $tmpA1 $channeling $clipping \
		-level ${invals},${gammaval}${unit} -clamp \
		+level ${outvals}${unit} \
		$outfile
